...
@admin_router.callback_query(
    NotifyUsersSG.get_target_language_code,
    NotifyUsersTargetLanguageFactory.filter(
...
async def get_notify_media(
    ...
    if message.text:
        ...
        await message.answer("❗️ Бот может рассылать только текст, фото, файлы, музыку, GIF и стикеры.")
        return
        ...
        try:
            for user in users:
                ...
                if not success[0] and success[1] == "bot_blocked":
                    await repo.users.update_user_active_status(telegram_id=user.telegram_id, is_active=False)
                else:
                    counter += 1
